...
impl MongoEngine {
    ...
    async fn put_item_impl(
        ...
        if let Some(capture) = stream {
            self.write_stream_inline_in_session(
                key_info, capture, old_item.as_ref(), Some(&item), &mut session,
            )
            .await?;
        }
    ...
    async fn delete_item_impl(
        ...
        if let Some(capture) = stream {
            self.write_stream_inline_in_session(
                key_info, capture, deleted_item.as_ref(), None, &mut session,
            )
            .await?;
        }
    ...
    async fn update_item_impl(
        ...
        for _attempt in 0..50 {
            ...
            // Sync GSI collections within the transaction
            self.sync_indexes_in_session(key_info, old_item.as_ref(), Some(&new_item), &mut session)
                .await?;

            // Write stream record within the transaction
            if let Some(capture) = stream {
                self.write_stream_inline_in_session(
                    key_info, capture, old_item.as_ref(), Some(&new_item), &mut session,
                )
                .await?;
            }
    ...
    async fn write_stream_inline_in_session(
        ...
        let size =
            source_item.map_or(0, |i| i64::try_from(item_size_bytes(i)).unwrap_or(i64::MAX));
